...
class RoundManager:
  ...
  @classmethod
  def start_new_round(self, round_count, small_blind_amount, big_blind_amount, table,initial_stack,cheat_deck):
    _state = self.__gen_initial_state(round_count, small_blind_amount,big_blind_amount, table)
    state = self.__deep_copy_state(_state)
    table = state["table"]

    for i in range(len(table.seats.players)):
      table.seats.players[i].stack = initial_stack
    table.deck.shuffle()
    # if cheat_deck:
    #   cids = [Card.from_str(cid).to_id() for cid in cheat_deck]
    #   cids.reverse()
    #   table.deck = Deck(cids)
    # self.__correct_ante(big_blind_amount, table.seats.players)
    self.__correct_blind(small_blind_amount,big_blind_amount, table)
  ...
  @classmethod
  def __correct_blind(self, sb_amount,bb_amount, table):
    self.__blind_transaction(table.seats.players[table.sb_pos()], True, sb_amount,bb_amount)
    self.__blind_transaction(table.seats.players[table.bb_pos()], False, sb_amount,bb_amount)

  @classmethod
  def __blind_transaction(self, player, small_blind, sb_amount,bb_amount):
    action = Const.Action.SMALL_BLIND if small_blind else Const.Action.BIG_BLIND
    blind_amount = sb_amount if small_blind else bb_amount
  ...
  def __start_street(self, state):
    ...
    # next_player_pos = state["table"].next_ask_waiting_player_pos(state["table"].dealer_btn)
    # state["next_player"] = next_player_pos
    street = state["street"]
    if street == Const.Street.PREFLOP:
      next_player_pos = state["table"].next_ask_waiting_player_pos(state["table"].bb_pos())
      state["next_player"] = next_player_pos
      return self.__preflop(state)
    elif street == Const.Street.FLOP:
      next_player_pos = state["table"].next_ask_waiting_player_pos(state["table"].sb_pos())
      state["next_player"] = next_player_pos
  ...
  def __forward_street(self, state):
    ...
    if table.seats.count_ask_wait_players() <= 1: #not one ask,such as allin call or all player fold
      state["street"] += 1
  ...
  def __is_everyone_agreed(self, state):
    ...
    tmp = [p for p in players if self.__is_agreed(max_pay, p)]
    everyone_agreed = len(players) == len(tmp)
  ...
  @classmethod
  def __gen_initial_state(self, round_count, small_blind_amount,big_blind_amount, table):
    return {
        "round_count": round_count,
        "small_blind_amount": small_blind_amount,
        "big_blind_amount": big_blind_amount,
        "street": Const.Street.PREFLOP,
        "next_player": (table.dealer_btn + 1) % len(table.seats.players),
        "table": table
    }
