...
class EngineWrapper(object):
    ...
    def start_game(self, players_info, game_config,global_mananager,cheat_deck = None):
        self.config = game_config
        #change
        # cids = [Card.from_str(cid).to_id() for cid in cheat_deck]
        # cids.reverse()
        # table = Table(Deck(cids))
        # setup table
        table = Table()
    ...
    def update_game(self, action, bet_amount,global_mananager):
        state, msgs = RoundManager.apply_action(self.current_state, action, bet_amount)
        if state['street'] == Const.Street.FINISHED:
            for i in range(len(state['table'].seats.players)):
                state['table'].seats.players[i].accumulate += state['table'].seats.players[i].stack
            global_mananager.human_total_win += state['table'].seats.players[1].stack - self.config['initial_stack']
            state, new_msgs = self._start_next_round(
                    state['round_count']+1, self.config['blind_structure'], state['table'])
            # global_mananager.history_actions = []
            msgs += new_msgs
        self.current_state = state
        return _parse_broadcast_destination(msgs, self.current_state['table'])
    ...
    def _start_next_round(self, round_count, blind_structure, table,global_mananager=None):
        table.shift_dealer_btn()
        table.dealer_btn = table.bb_pos()   #big_blind is dealer. next player is small blind
        small_blind, big_blind = _get_forced_bet_amount(round_count, blind_structure)
        # table = _exclude_short_of_money_players(table, ante, small_blind)
        # table = _exclude_short_of_money_players(table)
        if self._has_game_finished(round_count, table, self.config['max_round']):
            ...
            '''one round finish'''
            return RoundManager.start_new_round(round_count, small_blind, big_blind, table,self.config['initial_stack'],None)
...
def gen_game_config(max_round, initial_stack, small_blind, big_blind, blind_structure=None):
    assert max_round > 0
    assert initial_stack > 0
    assert small_blind > 0
    assert big_blind >= 0
    if not blind_structure:
        blind_structure = { 1 : { 'small_blind': small_blind, 'big_blind': big_blind } }
...
def _get_forced_bet_amount(round_count, blind_structure):
    ...
    return current_structure['small_blind'], current_structure['big_blind']

# def _exclude_short_of_money_players(table, ante=None, sb_amount=None):
