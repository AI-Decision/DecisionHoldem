        ...
        for destination, update in game_manager.latest_messages:
            ...
            for uuid in _parse_destination(destination, game_manager, sockets):
                if len(str(uuid)) <= 2:
                    ...
                    message = _gen_game_update_message(handler, update, game_manager.history_actions, game_manager.human_total_win)
                    if update['message']['message_type'] == 'round_start_message':
                        game_manager.history_actions = []
                    try:
                        socket.write_message(message)
                    except:
                        logging.error("Error sending message", exc_info=True)
                    time.sleep(_calc_wait_interval(mode, update))
                    if update['message']['message_type']=='round_result_message':
                        game_manager.latest_messages = game_manager.latest_messages[i:]
                        game_manager.isSendMessgae = False
                        return
...
def _find_socket_by_uuid(sockets, uuid):
    ...
    if len(target) != 1:
        raise Exception("error target uuid",target)
    return target[0]

def _gen_game_update_message(handler, message,history_actions=None,human_total_win = 0):
    ...
    if 'round_start_message' == message_type:
        ...
        try:
            ...
            action = None
        table_html_str = handler.render_string("round_state.html", round_state=round_state, action=action,isopen = False,human_total_win=human_total_win)
        ...
        event_html_str = handler.render_string(
                "event_update_game.html", action=action, round_state=round_state, history_actions= history_actions)
        content = {
                'update_type': message_type,
                'table_html': tornado.escape.to_basestring(table_html_str),
                'event_html': tornado.escape.to_basestring(event_html_str)
        ...
        round_count = message['message']['round_count']
        action = None
        table_html_str = handler.render_string("round_state.html", round_state=round_state, action=action,isopen = True,human_total_win=human_total_win)
        event_html_str = handler.render_string("event_round_result.html",
                round_state=round_state, hand_info=hand_info, winners=winners, round_count=round_count, history_actions= history_actions)
        content = {
                'update_type': message_type,
                'table_html': tornado.escape.to_basestring(table_html_str),
                'event_html': tornado.escape.to_basestring(event_html_str)
        ...
        event_html_str = handler.render_string("event_ask_action.html",
                ...
                action_histories=history_actions)
        content = {
                'update_type': message_type,
                'table_html': tornado.escape.to_basestring(table_html_str),
                'event_html': tornado.escape.to_basestring(event_html_str)
